...
export function DashboardPage() {
    ...
    return (
        <div className="max-w-5xl text-slate-200">
            <div className="mb-8">
                <h1 className="text-3xl font-bold text-white">
                    Welcome back{profile?.fullName ? `, ${profile.fullName.split(' ')[0]}` : ''}
                </h1>
                <p className="text-slate-400">Prepare for your next interview.</p>
            </div>

            {/* Active Session Alert */}
            {activeSession && (
                <div className="bg-surface border-2 border-primary/20 p-5 rounded-xl mb-8 flex items-center justify-between shadow-lg shadow-primary/5">
                    <div>
                    ...
                    {activeSession.status === 'active' && (
                        <Link
                            ...
                            className="ml-3 px-4 py-2 border border-primary text-primary rounded-lg text-sm font-medium hover:bg-primary/10 transition-all"
                        >
                            Live Console
                        </Link>
                    )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                <div className="bg-surface p-6 rounded-xl border border-white/5 shadow-sm">
                    ...
                    {credits === 0 && !loading && (
                        <Link to="/pricing" className="text-xs text-red-400 hover:underline mt-1 inline-block">
                            Buy more credits →
                        </Link>
                    )}
                </div>
                ...
                <div className="bg-surface p-6 rounded-xl border border-white/5 shadow-sm">
                    <div className="text-sm text-slate-400 font-medium mb-2">Avg. Score</div>
                    <div className="text-3xl font-bold text-white">
            ...
            <div className={`grid grid-cols-1 md:grid-cols-2 ${isAdmin ? 'lg:grid-cols-3' : ''} gap-6 mb-8`}>
                ...
                <Link
                    ...
                    className="bg-surface border border-white/5 p-6 rounded-xl shadow-sm hover:border-primary/30 transition-all group"
                >
                    <div className="flex items-center justify-between mb-4">
            ...
            {recentSessions.length > 0 && (
                <div className="bg-surface rounded-xl border border-white/5 shadow-sm overflow-hidden">
            ...
            {/* No Credits Warning */}
            {
                credits === 0 && !loading && (
                    <div className="mt-8 bg-amber-500/10 border border-amber-500/30 rounded-xl p-6 text-center">
                        ...
                        <h3 className="text-lg font-semibold text-amber-300 mb-2">You're out of credits</h3>
                        <p className="text-amber-200/80 mb-4">Purchase more credits to continue practicing interviews.</p>
                        <Link
                            to="/pricing"
                            className="inline-flex items-center gap-2 px-6 py-3 bg-amber-500 text-white font-semibold rounded-lg hover:bg-amber-600 transition-all"
                        >
                            Get More Credits
                        </Link>
                    </div>
